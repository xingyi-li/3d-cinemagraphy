...
class PartialLinearNoiseLayer(nn.Module):
    def __init__(self, opt, noise_sz=20, output_sz=32):
        ...
        self.pbn = partial_bn(output_sz)

        self.noise_sz = noise_sz
        self.opt = opt

    def forward(self, x, mask):
        if "bn_noise_misc" in self.opt and self.opt.bn_noise_misc:
            noise = torch.zeros(x.size(0), self.noise_sz).to(x.device)
        else:
        ...
        xp = self.pbn(x, mask, gain=gain, bias=bias)
        # x: [bs, c, W, H]
        # noise: [1, 20]
        # gain: [1, c, 1, 1]
        # bias: [1, c, 1, 1]
...
def get_D_norm_layer(config, norm_type="instance"):
...
def fused_bn(x, mean, var, gain=None, bias=None, eps=1e-5):
    ...
    # Prepare scale
    scale = torch.rsqrt(var + eps)  # [1, c, 1, 1]
    # If a gain is provided, use it
    if gain is not None:  # gain comes from fc(noise_z)
        scale = scale * gain
    # Prepare shift
    shift = mean * scale  # [1, c, 1, 1]
    # If bias is provided, use it
    if bias is not None:  # bias comes from fc(noise_z)
        shift = shift - bias
...
class partial_bn(nn.Module):
    ...
    def forward(self, x, mask, gain, bias):
        """
        x:[1, c, 256, 256]
        gain:[1, c, 1, 1]
        bias:[1, c, 1, 1]
        """
        if self.training:
            out, mean, var = partial_manual_bn(
                x, mask, gain, bias, return_mean_var=True, eps=self.eps
            )
            # If accumulating standing stats, increment them
            with torch.no_grad():
                if self.accumulate_standing:
        ...
        # If not in training mode, use the stored statistics
            ...
            if self.accumulate_standing:
                mean = mean / self.accumulation_counter
                var = var / self.accumulation_counter
            return fused_bn(x, mean, var, gain, bias, self.eps), mask


# Manual BN
# Calculate means and variances using mean-of-squares minus mean-squared

def partial_manual_bn(x, mask, gain=None, bias=None, return_mean_var=False, eps=1e-5):
    # Cast x to float32 if necessary
    float_x = x.float()
    # Calculate expected value of x (m) and expected value of x**2 (m2)
    # Mean of x
    ...
    # Mean of x squared
    # m2 = torch.mean(float_x ** 2, [0, 2, 3], keepdim=True)
    m2 = torch.sum(float_x ** 2, [0, 2, 3], keepdim=True) / (torch.sum(mask, [0, 2, 3], keepdim=True) + eps)
    ...
    # Return mean and variance for updating stored mean/var if requested
    if return_mean_var:
        return partial_fused_bn(x, m, var, gain, bias, eps), m.squeeze(), var.squeeze()
    else:
        return partial_fused_bn(x, m, var, gain, bias, eps)


def partial_fused_bn(x, mean, var, gain=None, bias=None, eps=1e-5):
    # Apply scale and shift--if gain and bias are provided, fuse them here
    # Prepare scale
    scale = torch.rsqrt(var + eps)  # [1, c, 1, 1]
    # If a gain is provided, use it
